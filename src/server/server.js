...
app.get("/api/stockPrices/historical", async (req, res) => {
  try {
    ...
    const stockDataPromises = symbols.map(symbol => yahooFinance.historical(symbol, { period1: '1900-01-01', interval: "1mo" }));
...
app.get("/api/stockPrices/insights", async (req, res) => {
    try {
      ...
      // Fetch chart data for each symbol
      const stockDataPromises = symbols.map(symbol => yahooFinance.quote(symbol));
      const stockData = await Promise.all(stockDataPromises);
